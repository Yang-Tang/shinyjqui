...
orderInput('A', NULL, items = 1:3, connect = 'B')
# items in B can be dragged to A
orderInput('B', NULL, items = 4:6, connect = 'A')
```
...
```{r, eval=FALSE}
# drag items in A to B by moving
orderInput('A', NULL, items = 1:3, connect = 'B')
# drag items in B to A by copying (source mode)
orderInput('B', NULL, items = 4:6, connect = 'A', as_source = TRUE)
```

If `placeholder` is set, `orderInput` shows it when there is no item in it:

```{r, eval=FALSE}
orderInput('A', 'An orderInput', items = 1:3, connect = 'B')
orderInput('B', 'An empty orderInput', items = NULL, placeholder = 'Drag item here...')
```
