...
func_intro <- data.frame(Functions = c('jqui_draggable', 'jqui_draggabled',
                         ...
                         Description = c('Attach draggable interaction to shiny html elements or manipulate it.',
                                         'Create a draggable shiny html element.',
                                         'Attach droppable interaction to shiny html elements or manipulate it.',
                                         'Create a droppable shiny html element.',
                                         'Attach resizable interaction to shiny html elements or manipulate it.',
                                         'Create a resizable shiny html element.',
                                         'Attach selectable interaction to shiny html elements or manipulate it.',
                                         'Create a selectable shiny html element.',
                                         'Attach sortable interaction to shiny html elements or manipulate it.',
                                         'Create a sortable shiny html element.'),
                         Where_to_use = rep(c('server', 'ui'), times = 5),
                         stringsAsFactors = FALSE)
knitr::kable(func_intro, row.names = FALSE)
```

For each kind of interaction, there are two functions: `-able()` and `-abled()`. They both can be used to add interaction to a shiny html element, but should be used in different context. The `-abled()` functions are designed to be used in `ui` of a shiny app. They return a modified shiny html element (tag, tagList, input or output) with interaction enabled. The `-able()` functions modify the interactivity of existing (pre-defined in `ui`) elements and therefore should be placed in `server` of a shiny app. For example, you can make a draggable `textInput` either by `jqui_draggabled()`:

```{r, eval = FALSE}
# in shiny ui
jqui_draggabled(textInput('foo', 'Input'))

```

or by `jqui_draggable()`:
...
jqui_draggable(selector = '#foo')
```

The `-able()` functions are more flexible in the following three ways:

1. By calling them inside an `observe()` or `ovservEvent()`, you can reactively control the element's interactivity .
2. They are more flexible in selecting the target shiny html element(s). The `selector` argument accepts either a string of [jQuery selector](https://api.jquery.com/category/selectors/) or a `JS()` wrapped JavaScript expression that returns a jQuery object. In the above example, which used the jQuery selector mode, the string `#foo` refers to the element with id `foo`. To select a set of elements, use their `class name`, e.g., use `jqui_draggable(selector = ".shiny-bound-input")` to make all the shiny inputs draggable. The JavaScript expression mode gives more freedom to make your selection. You can write your own JavaScript expression that returns a jQuery object to refer to any element(s) you want, e.g., use `jqui_resizable(selector = JS("$('#foo').parent()"))` to make the parent of element with id `foo` resizable.
3. In addition to `enable` the interactions, the `-able()` functions also support `disable`, `destory`, `save` and `load` the interactions. This behavior is controlled by the `operation` argument with the default value `enable`. The difference between `disable` and `destory` is that the `disable` operation only disable the effect of interaction temporarily without removing the corresponding settings, whereas the `destory` operation removes the interaction totally. The `save` and `load` operations are used to save and restore the interaction state (e.g., the current position of a draggable element). Please see the vignette `Save and restore` for more details.

To make multiple elements interactive, you can either use `-abled()` functions combining with `tagList()`:
...
or use jQuery selector in `-able()` functions:

```{r, eval = FALSE}
jqui_draggable("#sel,#chbox,#plot")
```

Both `-able()` and `-abled()` functions have an `options` argument to accept interaction-specific options. The full list of the available options can be found in jQuery UI's  [API Documentation](http://api.jqueryui.com/category/interactions/) page. The example sections below showed how to use them.
...
The shiny option list is composed by multiple `suffix` defining units. The unit name is used as `suffix` in `input$<id>_<suffix>`. Within each unit, there are multiple `JS()` wrapped JavaScript callback functions named with corresponding event types. The event triggers the callback and the returned value is send to server in the form of `input$<id>_<suffix>`. The valid event types for each interactions can be found in the [API Documentation](http://api.jqueryui.com/category/interactions/) of jQuery UI. The callback functions take two arguments, `event` and `ui`. The definition of the `ui` argument can also be found in the document too. Here is an example:
